...
/*   Updated: 2024/01/16 06:46:49 by maouzal          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_my_mlx_pixel_get_animated(t_mlx *mlx, int x, int y, int i)
{
	char	*dst;
	if (mlx->win_x > x && mlx->win_y > y && x >= 0 && y >= 0)
	{
		dst = mlx->animation[i].id
			+ (y * mlx->animation[i].len + x * (mlx->animation[i].bpp / 8));
		return (*(unsigned int *)dst);
	}
	return (0);
}
